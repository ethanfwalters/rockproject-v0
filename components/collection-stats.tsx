...
export function CollectionStats({ specimens }: CollectionStatsProps) {
  const stats = useMemo(() => {
    ...
    const recentAdditions = specimens.filter((s) => new Date(s.dateAdded) >= thirtyDaysAgo).length
      ...
      <Card className="border-0 bg-card p-6">
        ...
        <div className="space-y-3">
          <div className="flex justify-between">
            <span className="text-sm text-muted-foreground">Added last 30 days</span>
            <span className="text-sm font-medium">{stats.recentAdditions}</span>
          </div>
          {stats.oldestSpecimen && (
            <div className="mt-4 pt-4 border-t">
              <p className="text-xs text-muted-foreground mb-1">Oldest Specimen</p>
              <p className="text-sm font-medium">{stats.oldestSpecimen.name}</p>
              <p className="text-xs text-muted-foreground">
                {new Date(stats.oldestSpecimen.dateAdded).toLocaleDateString()}
              </p>
            </div>
          )}
        </div>
